  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-8 flex flex-col items-center">
        <div className="z-10 my-10 flex flex-1 cursor-pointer items-center gap-2 rounded-xl bg-slate-900 p-4 backdrop-blur hover:animate-flip">
          <HandPalm className="text-red-600" size={32} />
          {backendSkills.map((skill) => (
            <span key={skill.id}>{skill.name}</span>
          ))}
        </div>
        <div className="z-10 my-10 flex flex-1 -skew-y-6 gap-2 rounded-xl bg-slate-900 p-4 backdrop-blur">
